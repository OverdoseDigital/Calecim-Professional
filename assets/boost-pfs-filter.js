...
var boostPFSFilterConfig = {
  search: {
		...
		highlightSuggestionResult: false
	},
  general: {
    ...
    forceCapitalizeFilterOptionValues: false,
    capitalizeFirstLetterFilterOptionValues: false,
    moneyFormatWithCurrency: true,
    // Placeholder  
...
ProductGridItem.prototype.compileTemplate = function(data) {
  ...
  itemHtml = itemHtml.replace(/{{itemCustomTagLabels}}/g, buildCustomTagLabels(data, false));
  // Add Images
  itemHtml = itemHtml.replace(/{{itemImages}}/g, buildImages(data));
  // Add Price
  itemHtml = itemHtml.replace(/{{itemPrice}}/g, buildPrice(data));
  itemHtml = itemHtml.replace(/{{itemPriceDesktop}}/g, buildPriceDesktop(data));
  itemHtml = itemHtml.replace(/{{itemColor}}/g, buildColor(data));
  itemHtml = itemHtml.replace(/{{itemPriceMobile}}/g, buildPriceMobile(data));
...
function buildPriceDesktop(data) {
    var html = "";
    if(data.price_max > data.price_min) {
        html += `<span class="p2 bold-500 c-grey">${Utils.formatMoney(data.price_min)}</span>`;
        html += `<span class="p2 bold-500 c-grey show-d second-price">&nbsp;- ${Utils.formatMoney(data.price_max)}</span>`;  
    } else {
        html += `<span class="p2 bold-500 c-grey">${Utils.formatMoney(data.price_min)}</span>`; 
        if (onSale) {
          html += `<span class="p2 bold-500 c-grey show-d second-price opacity-5 ml-5"><strike>${Utils.formatMoney(data.compare_at_price_min)}</strike></span>`;
        } 
    }
...
function buildColor(data) {
    var html = "";

    const options_with_val = data.options_with_values;
    let pro_color = "";
    let color_array = [];

    //console.log(data)

    html += '<div class="color-variant-picker mt-15 width-100">';
    for(let i=0; i<options_with_val.length; i++) {
        if(options_with_val[i].label.includes("Colour") || options_with_val[i].label.includes("Color")) {
            for(let j=0; j<options_with_val[i].values.length; j++) {
              color_array.push(options_with_val[i].values[j].title);
            }
        }
    }
    ...
    html += `<div class="swatch-label hide-m show-d variant-each-label p4 uppercase">${color_array[0]}</div>`;
    html += '<div class="swatch-wrapper variant-each-wrapper mt-5 global-flex flex-wrap al-center">';
    for(let i=0; i<color_array.length; i++) {
        let featured_image = "";
        for(let j=0; j<data.variants.length; j++) {
            if(data.variants[j].option1 == color_array[i] || data.variants[j].option2 == color_array[i] || data.variants[j].option3 == color_array[i]){
                if(data.variants[j].image != null){
                    featured_image = data.variants[j].image;
                    j = data.variants.length;
                }
            }
        } 
        if(i == 0){
            html += `<div onclick='productEachColorPickerCollection(event,"${featured_image}")' class="swatch variant-each-button active" data-index="${i + 1}" data-image="" data-value="${color_array[i]}">`;
        }else{
            html += `<div onclick='productEachColorPickerCollection(event,"${featured_image}")' class="swatch variant-each-button" data-index="${i + 1}" data-image="" data-value="${color_array[i]}">`;
        }
        
        html += `<div class="swatch-inner swatch-${handleColorSwatch(color_array[i])}"></div>`;
        html += '</div>';
    }


    html += `<div class="swatch-label show-m-inline-block hide-d variant-each-label p4 uppercase y1">${color_array[0]}</div>`;
    html += '</div>';
    html += '</div>';
...
function handleColorSwatch(data) {
    return data.replace(/[!@#$%^&*()]/g, "").replace(/\s+/g, "-").toLowerCase();
}

function buildPriceMobile(data) {
  var html = "";

  // price on mobile only
  html += `<div class="show-m hide-t mt-5 price-container-mobile">`;
    
  let color_class = 'c-black';

  if(onSale) {
    html += `<span class="p3 mr-5 show-m c-red">${Utils.formatMoney(data.price_min)}</span>`;
    html += `<span><strike class="p3 c-text-grey">${Utils.formatMoney(data.compare_at_price_min)}<strike></span>`;
  } else {
    html += `<span class="p3 mr-5 show-m ${color_class}">${Utils.formatMoney(data.price_min)}</span>`;
  }
...
// Build custom tag labels
function buildCustomTagLabels(data) {
  var html = `<div class="product-labels-container ab-top-right z-3 pt-10 pl-10 d-pt-15 d-pl-15">`;

  var arr_sticker_enabled = boostPFSAppConfig.general.isEnableSticker;
	var arr_sticker_tags = boostPFSAppConfig.general.stickerTags.split('|');
	var arr_sticker_bgs = boostPFSAppConfig.general.stickerBgs.split('|');
	var arr_sticker_colors = boostPFSAppConfig.general.stickerColors.split('|');

  var lowStockEnable = boostPFSAppConfig.general.lowStockEnable;
  var lowStockThreshold = boostPFSAppConfig.general.lowStockThreshold;
  var lowStockLabel = boostPFSAppConfig.general.lowStockLabel;
  var lowStockBg = boostPFSAppConfig.general.lowStockBg;
  var lowStockColor = boostPFSAppConfig.general.lowStockColor;

  //-- sale sticker
  if (onSale) {
    const salePercent = Math.round((data.compare_at_price_min - data.price_min) * 100 / data.compare_at_price_max);
    //html += `<div class="global-product-label bg-white sale-label mr-10 mb-10"><span class="c-red p4 uppercase tc">Sale</span></div>`;
  }

  //-- custom stickers
  if(arr_sticker_enabled) {
    for(var i=0; i<arr_sticker_tags.length; i++) {
      if(arr_sticker_tags[i] != '' && arr_sticker_bgs[i] != '' && arr_sticker_colors[i] != '' && data.tags.includes(arr_sticker_tags[i])) {
        let tag_prefix = arr_sticker_tags[i].split(":");
        //const this_tag = arr_sticker_tags[i];
        let this_tag = "";
        if(tag_prefix.length > 1){
          this_tag = tag_prefix[1];
        }else{
          this_tag = tag_prefix[0];
        }
        html += `<div class="global-product-label" style="background: ${arr_sticker_bgs[i]};"><span class="p3 bold-500 uppercase tc spacing-2" style="color: ${arr_sticker_colors[i]}">${this_tag}</span></div>`;
      } 
    }
  }
  ...
  //-- out of stock
  if(!data.available) {
    html += `<div class="global-product-label bg-red"><span class="p3 bold-500 uppercase tc c-white spacing-2">Out of Stock</span></div>`;
  }
...
ProductSorting.prototype.bindEvents = function() {
  ...
  //-- clear all event
  jQ('.boost-pfs-filter-mobile-footer').off().on('click', '.clear-btn-mobile', function(){
    jQ('.boost-pfs-filter-refine-by-wrapper .boost-pfs-filter-clear-all').trigger('click');
  });
};
...
ProductList.prototype.afterRender = function(data) {
  ...
  //-- remove cta blocks
  //$('.product-list-wrapper .boost-pfs-filter-products').find(`div[id*="cta-each-"]`).remove();

  //-- change limit
  //if(window.innerWidth >= 1024) {
  //  Globals.queryParams.limit = 15;
  //}
  Globals.queryParams.limit = 16;
  //-- render cta blocks port to native done
  /*$(document).find('.cta-blocks-container .cta-each .boost-pfs-filter-product-item').each(function(){
    let cta_index = parseInt($(this).parent().attr('data-index'));
    let cta_index_place = cta_index;

    if(cta_index_place == 9) {
      if($(window).width() < 768) {
        cta_index_place = 8;
      }
    }

    let elem = $(`#cta-each-${cta_index}`).parent().html();
    if($('.product-list-wrapper .boost-pfs-filter-products').find(`#cta-each-${cta_index}`).length <= 0) {
      $(elem).insertBefore(`.boost-pfs-filter-products .boost-pfs-filter-product-item:nth-child(${cta_index_place})`);
    }
    ...
    if (data.length){
      const ctas = document.querySelectorAll('.cta-blocks-container .cta-each .boost-pfs-filter-product-item');
      ctas.forEach(cta => {
          let cta_index = parseInt(cta.parentElement.getAttribute('data-index'));
          let cta_index_place = cta_index;
          let elem = document.querySelector(`#cta-each-${cta_index}`);

          if(document.querySelector(`.product-list-wrapper .boost-pfs-filter-products #cta-each-${cta_index}`)){

          }else{
              let parentElement = document.querySelector(".product-list-wrapper .boost-pfs-filter-products");
              let referenceElement = document.querySelector(`.product-list-wrapper .boost-pfs-filter-products .boost-pfs-filter-product-item:nth-child(${cta_index_place}`);
              let copiedElement = elem.cloneNode(true);
              parentElement.insertBefore(copiedElement, referenceElement);
...
Filter.prototype.afterRender = function(data) {
  ...
  //-- move refine by and sort on mobile, clear button port to native
  if(window.innerWidth < 1024) {
    /*const clear_button_mobile = `<a href="javascript:void(0)" class="btn3 clear-btn-mobile tc">Clear <span class></span></a>`;

    if($('.boost-pfs-filter-tree-content .boost-pfs-filter-mobile-footer .clear-btn-mobile').length <= 0) {
      $(clear_button_mobile).prependTo($('.boost-pfs-filter-tree-content .boost-pfs-filter-mobile-footer'));
    }*/

    const clear_button_mobile = document.createElement('button');
    clear_button_mobile.classList.add("btn3");
    clear_button_mobile.classList.add("clear-btn-mobile");
    //clear_button_mobile.innerHTML = '<a href="javascript:void(0)" class="btn3 clear-btn-mobile tc">Clear</a>';'
    clear_button_mobile.innerHTML = 'Clear';
    if(document.querySelector('.boost-pfs-filter-tree-content .boost-pfs-filter-mobile-footer .clear-btn-mobile')){

    }else{
      const parentElement = document.querySelector('.boost-pfs-filter-tree-content .boost-pfs-filter-mobile-footer');
      console.log(parentElement)
      console.log(clear_button_mobile)
      parentElement.prepend(clear_button_mobile);
    }
  }

    const online_button = document.querySelector(".filter-outer-wrapper .online-toggle");
    const filters = document.querySelectorAll(".boost-pfs-filter-option-list");
    let online_available = false;
    filters.forEach(filter => {
        const filter_name = filter.querySelector(".boost-pfs-filter-option-title-heading").getAttribute("aria-label");

        if (filter_name == "Filter by Online"){
          online_available = true;
          filter.classList.add("hidden");
        }
    });
    
    //const last_filter = document.querySelector(".boost-pfs-filter-options-wrapper").lastElementChild.querySelector(".boost-pfs-filter-option-title-heading");
    //const last_filter_name = last_filter.getAttribute("aria-label");
    
    if (!online_available){
      online_button.style.display = "none";
    }else{
      if(window.innerWidth >= 1024) {
        online_button.style.display = "flex";
      }
    }

    if(!document.querySelector('.online-toggle.mobile') && online_available && window.innerWidth < 1024){
      // get the element after which we want to insert
      const referenceElement = document.querySelector('.boost-pfs-filter-mobile-toolbar');

      // create the HTML content to insert
      const htmlContent = `<div class="flex online-toggle mobile jc-between bb-1 b-silver">
      <div class="p2 pr-10 bold-500" style="color:#2c2c2c;">Online Exclusive</div>
      <div class="online-button cursor" onclick='toggleOnlineExclusiveMobile(event)'><div class="online-button-inner"></div></div>
      </div>`;

      // insert the HTML content after the reference element
    ...
    if (window.location.href.includes('pf_t_online=category%3AOnline+Exclusive')) {
        if(document.querySelector('.online-toggle.mobile')){
            document.querySelector('.online-toggle.mobile').classList.add('active');
        }
    }
